...
function provisioning_start() {
    ...
    provisioning_get_models "${WORKSPACE}/ComfyUI/models/ultralytics/bbox" "${YOLO_MODELS[@]}"
    provisioning_get_models "${WORKSPACE}/ComfyUI/models/sams" "${SAMS[@]}"
...
function provisioning_get_models() {
    ...
    for item in "${arr[@]}"; do
        if [[ "$item" == *~* ]]; then
            ...
            provisioning_download "$url" "$dir" "$key"
        else
            printf "Processing key-only model: %s\n" "$item"
...
function provisioning_download() {
    ...
    if [[ -n $auth_token ]]; then
        if [[ -n $3 ]]; then
            echo 'here'
            curl -L -H "Authorization: Bearer $auth_token" --progress-bar -o "$3" "$1"
        else
            curl -L -H "Authorization: Bearer $auth_token" --progress-bar -OJ "$1"
        fi
    else
        if [[ -n $3 ]]; then
            curl -L --progress-bar -o "$3" "$1"
        else
            curl -L --progress-bar -OJ "$1"
        fi
    fi
}
