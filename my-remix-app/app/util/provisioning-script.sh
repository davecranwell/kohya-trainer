...
CHECKPOINT_MODELS=(
    ### BEGIN_CHECKPOINT_MODELS ###
    ### END_CHECKPOINT_MODELS ###
...
# Download from $1 URL to $2 file path but only if the file is not already present (-nc = no clobber)
function provisioning_download() {
    ...
    if [[ -n $auth_token ]];then
        wget --header="Authorization: Bearer $auth_token" -qnc --content-disposition --show-progress -e dotbytes="${3:-4M}" -P "$2" "$1" -O "$2/$3"
    else
        wget -qnc --content-disposition --show-progress -e dotbytes="${3:-4M}" -P "$2" "$1" -O "$2/$3"
    fi
}
