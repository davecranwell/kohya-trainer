...
app.use(
    helmet({
        ...
        contentSecurityPolicy: {
            ...
            reportOnly: true,
            directives: {
                'connect-src': [MODE === 'development' ? 'ws:' : null, process.env.SENTRY_DSN ? '*.sentry.io' : null, "'self'"].filter(Boolean),
                'font-src': ["'self'"],
                'frame-src': ["'self'"],
                'img-src': ["'self'", 'data:', 'https://sdscriptloratraining.s3.us-east-1.amazonaws.com'],
                'script-src': [
                    "'strict-dynamic'",
                    "'self'",
                    // @ts-expect-error
