...
exports.handler = async (event) => {
    ...
    for (const record of event.Records) {
        ...
        const imageObject = await s3.send(new GetObjectCommand({ Bucket: process.env.SOURCE_BUCKET_NAME, Key: imageUrl }));
        ...
        for (const size of sizes) {
            ...
            const key = size.getKey(uploadedKey);
        ...
        // now fire a webhook back to the app site to update the status of processing the image
